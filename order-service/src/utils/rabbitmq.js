// Shared RabbitMQ utility (same pattern as user-service)
...
async function publishEvent(routingKey, data) {
  if (!channel) return;
  try {
    const msg = JSON.stringify({ routingKey, data, timestamp: new Date().toISOString() });
    channel.publish('shopnest.events', routingKey, Buffer.from(msg), { persistent: true });
  } catch (err) {
